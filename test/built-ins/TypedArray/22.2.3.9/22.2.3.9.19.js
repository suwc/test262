...
/*---
description: Add elements to the typed array in filterFn
...
var filterObj = {
    ...
    filterFn(value, index, obj) {
        ...
        Object.defineProperty(obj, (array.length + this.arrayIndex).toString(), {
            ...
            configurable: false,
            writable: true,
            value: array[array.length / 2 + this.arrayIndex]
        });
...
for (i = 0; i < typedArrays.length; i++) {
    ...
    for (j = 0; j < array.length / 2; j++) {
        if (array[j * 2] !== filteredArrays[i][j]) {
            $ERROR("Element mismatch for Filtered array of " + filteredArrays[i][Symbol.toStringTag] + " at index " + j + ". Actual : " + filteredArrays[i][j] + ". Expected : " + array[j * 2]);
        }
    }
}
