...
for (i = 0; i < typedArrays.length; i++) {
    savedPrev = null;
    var output = typedArrays[i].reduce(callbackObj.callbackFn);
    if (output != outputObj) {
        $ERROR("Reduce was expected to return " + outputObj + " but returned " + output + " for " + typedArrays[i][Symbol.toStringTag]);
    }
}
