...
for (i = 0; i < typedArrayTypes.length; i++) {
    ...
    var output = typedArray.findIndex(predicateObj.predicate, predicateObj);
    if (output !== -1) {
        $ERROR("findIndex was expected to return -1 but returned " + output + " for " + typedArray[Symbol.toStringTag]);
    }
}
